...
class Controller():
    ...
    def get_items(self, message: types.Message, all_shops: bool = False) -> Dict[str, Optional[List[Item]]]:
        res = {}
        numbers = self.get_shop_item_number(message, all_shops)
        for shop in self.shops:
            if all_shops or not self.shops[shop]:
                res[shop.name] = shop.get_items(request=message.text, number=numbers[shop])
    ...
    def add_shop_item_number(self, message: types.Message) -> None:
        if int(message.text) <= 0:
            raise ValueError
        self.db.add(message.chat.id, self.chosen_shop, int(message.text))

    def get_shop_item_number(self, message: types.Message, all_shops: bool = False) -> DefaultDict[str, int]:
        return defaultdict(lambda: 2 if all_shops else 5, self.db.get_shop_item_numbers(message.chat.id))